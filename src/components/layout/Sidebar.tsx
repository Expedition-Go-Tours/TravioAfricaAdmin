...
function getNavGroups(can: (key: string) => boolean): { group: string; items: NavItem[] }[] {
  const groups: { group: string; items: NavItem[] }[] = [
    ...
    ...(can('suppliers.view') || can('reviews.view') || can('bookings.view') || can('chat.suppliers') || can('blog.manage') || can('tours.approve') ? [{
      group: "Management",
      items: [
        ...(can('bookings.view') ? [{ label: "Bookings", path: "/admin/bookings", icon: <ShoppingCart className="h-4 w-4" />, badgeKey: "bookings" }] : []),
        ...
        ...(can('reviews.view') ? [{ label: "Reviews", path: "/admin/reviews", icon: <Star className="h-4 w-4" />, badgeKey: "reviews" }] : []),
        ...(can('tours.approve') ? [{ label: "Tour Moderation", path: "/admin/tour-moderation", icon: <ClipboardCheck className="h-4 w-4" />, badgeKey: "tours" }] : []),
